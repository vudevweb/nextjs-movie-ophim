...
const SlideCatalog = ({ data }) => {
     const uImg = data.APP_DOMAIN_CDN_IMAGE + '/uploads/movies/'
     const id_cate = data.type_list
               ...
               <div className="container">
                    <div className="row">
                         <div className="col-12">
                              <h2 className="section__title"><a href={data.breadCrumb[0].slug}>{data.titlePage}</a></h2>
                         </div>
                         <div className="col-12">
                              <div className="section__carousel-wrap">
                                   <div className="section__carousel owl-carousel" id={id_cate}>
                                        {movie.map((item, index) => (
                                             <div key={index} className="card">
                                                  <a href={`/phim/${item.slug}`} className="card__cover">
                                                       <img src={`${uImg+item.thumb_url}`} alt={item.name} height={270}/>
                                                  ...
                                                  <h3 className="card__title"><a href={`/phim/${item.slug}`}>{item.name}</a></h3>
